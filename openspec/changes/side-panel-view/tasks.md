...
- [x] 1.1 Create `src/views/side-panel-view.ts` with `RoutineSidePanelView extends ItemView` (view type `routine-side-panel`), subscribing to `plugin.store` on open and unsubscribing on close, mirroring `RoutineStatusBarItem`'s subscribe/unsubscribe lifecycle
...
- [x] 1.5 Apply `routine-side-panel` root class and reuse `routine-controls`/`routine-queue` class names

## 2. Registration and discoverability

- [x] 2.1 Register the view type in `main.ts` via `this.registerView('routine-side-panel', ...)`
- [x] 2.2 Add `activateView()` helper: reveal an existing `routine-side-panel` leaf if one exists, otherwise create one via `workspace.getRightLeaf(false)` and reveal it
...
- [x] 2.5 Unload/cleanup: ensure the view's store subscription is released in `onClose`, mirroring `RoutineStatusBarItem.unload()`
