...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```bash
   bun x openspec instructions archive --change "<name>" --json
   ```
...
2. **Check artifact completion status**
   ...
   Run `bun x openspec status --change "<name>" --json` to check artifact completion.
...
4. **Assess delta spec sync state**
   ...
   **Prompt options:**
   ...
   `bun x openspec instructions specs --change "<name>" --json` once with the same
...
5. **Perform the archive**
   ...
   Generate the target name: use the change name as-is when it already starts with a `YYYY-MM-DD-` prefix; otherwise prepend the current date as `YYYY-MM-DD-<change-name>`. Never stack a second date (same rule as `bun x openspec archive`).
...
- Use artifact graph (bun x openspec status --json) for completion checking
